...
@RequestScoped
public class DefaultProductOfferReverseRouter extends AbstractLocalizedReverseRouter implements ProductOfferReverseRouter {

    private final SimpleProductOfferReverseRouter delegate;

    @Inject
    protected DefaultProductOfferReverseRouter(final Locale locale, final SimpleProductOfferReverseRouter reverseRouter) {
        super(locale);
    ...
    @Override
    public Call productOfferPageCall(final String languageTag, final String categoryIdentifier) {
        return delegate.productOfferPageCall(languageTag, categoryIdentifier);
    }
     ...
     */
    @Override
    public Optional<Call> productOfferPageCall(final Category category) {
        return category.getSlug().find(locale())
                .map(this::productOfferPageCall);
    }
}
